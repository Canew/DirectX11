...
void SkyShader::Init(ID3D11Device& device)
{
	BuildShader(device);
}

void SkyShader::SetShader(ID3D11DeviceContext& deviceContext)
{
...
void SkyShader::BuildShader(ID3D11Device& device)
{
	...
	if (FAILED(D3DCompileFromFile(L"Shader/SkyVertexShader.hlsl", NULL, NULL, "main", "vs_5_0", shaderFlags, 0, vertexShaderBuffer.GetAddressOf(), errorMessage.GetAddressOf())))
	{
		if (errorMessage)
		{
	...
	if (FAILED(D3DCompileFromFile(L"Shader/SkyPixelShader.hlsl", NULL, NULL, "main", "ps_5_0", shaderFlags, 0, pixelShaderBuffer.GetAddressOf(), errorMessage.GetAddressOf())))
	{
		if (errorMessage)
		{
			OutputDebugStringA(reinterpret_cast<const char*>(errorMessage->GetBufferPointer()));
