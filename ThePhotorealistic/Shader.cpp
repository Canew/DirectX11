...
void Shader::SetShader(ID3D11DeviceContext& deviceContext)
{
	...
	deviceContext.PSSetSamplers(0, mSamplerList.size(), mSamplerList.data()->GetAddressOf());
}
...
void Shader::BuildShader(ID3D11Device& device)
{
	...
	if (FAILED(D3DCompileFromFile(L"Shader/BasicVertexShader.hlsl", NULL, &include, "main", "vs_5_0", shaderFlags, 0, vertexShaderBuffer.GetAddressOf(), errorMessage.GetAddressOf())))
	{
		if (errorMessage)
		{
	...
	if (FAILED(D3DCompileFromFile(L"Shader/BasicPixelShader.hlsl", NULL, &include, "main", "ps_5_0", shaderFlags, 0, pixelShaderBuffer.GetAddressOf(), errorMessage.GetAddressOf())))
	{
		if (errorMessage)
		{
			OutputDebugStringA(reinterpret_cast<const char*>(errorMessage->GetBufferPointer()));
